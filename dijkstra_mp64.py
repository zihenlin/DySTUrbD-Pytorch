...
def sharedSearch_D(
    indexList, dataset, shared_prd, shared_dist, idx, arrPos, sharedCSR, datashape
):
    """
...
def multiSearch(dataset, nprocs, returnShared=False):
    ...
    X_np = np.frombuffer(shared_prd, dtype=np.float64).reshape(datashape)
